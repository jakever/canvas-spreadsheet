...
class ColumnHeaderRow {
    ...
    draw() {
        ...
        const style = {
            borderColor: this.grid.borderColor,
            borderWidth: this.grid.borderWidth,
            fillColor: this.grid.bgColor
        }
        this.grid.painter.drawRect(ROW_HEADER_WIDTH, 0, CHECK_BOX_WIDTH, HEADER_HEIGHT, style)
        this.grid.painter.drawImage(offcheck, ROW_HEADER_WIDTH + (CHECK_BOX_WIDTH - 20) / 2, (HEADER_HEIGHT - 20) / 2, 20, 20)
        
        this.grid.painter.drawRect(0, 0, ROW_HEADER_WIDTH, HEADER_HEIGHT, style)
    }
}
