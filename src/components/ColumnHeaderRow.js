...
class ColumnHeaderRow {
  constructor(grid, x, y, columns) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (column.fixed === "left") {
        this.fixedLeftColumnHeaders.push(
          new ColumnHeader(grid, i, everyOffsetX, y, column, style)
        );
      } else if (column.fixed === "right") {
        this.fixedRightColumnHeaders.push(
          new ColumnHeader(grid, i, everyOffsetX, y, column, style)
        );
      } else {
  ...
  mouseMove(x, y) {
    if (this.isResizing) {
      ...
      this.grid.resizeColumn(
        this.resizeTarget.index,
        this.resizeOriginalWdith + resizeDiffWidth
      );
    } else {
      // 鼠标移动中 -> 寻找需要调整列宽的列目标
  ...
  draw() {
    ...
    this.grid.painter.drawImage(
      offcheck,
      ROW_HEADER_WIDTH + (CHECK_BOX_WIDTH - 20) / 2,
      (HEADER_HEIGHT - 20) / 2,
      20,
      20
    );
