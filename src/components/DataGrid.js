...
class DataGrid {
  constructor(target, options) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      this.rows.push(
        new Row(this, i, 0, everyOffsetY, rowHeight, this.columns, rowData)
      );
  ...
  setActualSize() {
    this.fixedLeftWdith = this.columnHeaderRow.fixedLeftColumnHeaders.reduce(
      (sum, col) => {
        return sum + col.width;
      },
      0
    );
    this.fixedRightWdith = this.columnHeaderRow.fixedRightColumnHeaders.reduce(
      (sum, col) => {
        return sum + col.width;
      },
      0
    );
    this.actualTableWidth = this.columnHeaderRow.columnHeaders.reduce(
      (sum, item) => {
  ...
  updateSelection(x, y) {
    if (this.selectorControl.isSelected) {
      const minX =
        x > this.editorControl.editorXIndex
          ? this.editorControl.editorXIndex
          : x;
      const maxX =
        x > this.editorControl.editorXIndex
          ? x
          : this.editorControl.editorXIndex;
      const minY =
        y > this.editorControl.editorYIndex
          ? this.editorControl.editorYIndex
          : y;
      const maxY =
        y > this.editorControl.editorYIndex
          ? y
      ...
      for (
        let i = this.selectorControl.selectedYArea[0];
        i <= this.selectorControl.selectedYArea[1];
        i++
      ) {
        this.rows[i].updateSelection(
          this.selectorControl.selectedXArea[0],
          this.selectorControl.selectedXArea[1]
        );
      }
    }
  }
  ...
  handleScroll(e) {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      let maxWidth = 0;
      if (this.fillCellWidth > 0) {
        // 列总宽小于容器宽
        maxWidth = this.actualTableWidth;
      } else {
        maxWidth =
          this.actualTableWidth + this.fixedLeftWdith + this.fixedRightWdith;
      }
